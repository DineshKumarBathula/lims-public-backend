module.exports = function AcidtileBreakingStrength(jdata) {
  const data = jdata.find((p) => p.param_id === "ACID_BREAKING_STRENGTH");
  if (!data) return [];
  ...
  const TEST_METHOD = "IS:4457 Annex-D";
  const requirements =
    group || specification
